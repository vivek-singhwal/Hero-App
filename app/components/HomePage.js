...
*/
  ...
  handleDiscoverPeripheral(peripheral) {
    ...
    if (typeof (peripheral) != undefined) {
      if (!peripherals.has(peripheral.id ) ) {
        //EventRegister.emit('BLE_STATUS', { event: "Connecting " + bleService.deviceName });
        //bleService.setPeripheral(peripheral);
        BleManager.stopScan()
          .then(() => {
  ...
  test(peripheral) {
    console.log("connected"+peripheral.connected);
    if (peripheral){
      if (peripheral.connected){
        ...
        BleManager.connect(peripheral.id).then(() => {
          ...
          console.log('Connected to ' + peripheral.id +"\n"+peripheral.localName);
              ...
              var bakeCharacteristic = 'D973F2E2-B19E-11E2-9E96-0800200C9A66';
              var crustCharacteristic = 'D973F2E1-B19E-11E2-9E96-0800200C9A66';
                ...
                BleManager.startNotification(peripheral.id, service, bakeCharacteristic).then(() => {
                  ...
                  setTimeout(() => {
                    BleManager.write(peripheral.id, service, crustCharacteristic, [1]).then(() => {
                        BleManager.read(peripheral.id, service, bakeCharacteristic, 1)
                        .then((readData) => {
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.container}>
            <NavigationEvents
            onWillFocus={payload => { Picker.hide() }}
            onDidFocus={payload => {
                if (payload.state.routeName == "HomePage") {
                this.setState({});
                }
            }}
            />
