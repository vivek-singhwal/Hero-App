        ...
        BleManager.connect(peripheral.id).then(() => {
          ...
          setTimeout(() => {
            BleManager.retrieveServices(peripheral.id).then((peripheralInfo) => {
              ...
              // var read = '6e400003-b5a3-f393-e0a9-e50e24dcca9e';
              // var uart = "6E400008-B5A3-F393-E0A9-E50E24DCCA9E";

              var service = 'D973F2E0-B19E-11E2-9E96-0800200C9A66';
              var write = 'D973F2E1-B19E-11E2-9E96-0800200C9A66';
              var read = 'D973F2E2-B19E-11E2-9E96-0800200C9A66';
              // setTimeout(() => {
                // BleManager.retrieveServices(peripheral.id).then((resServices)=>{
                //   console.log('retrieveServices ' + JSON.stringify(resServices));
                // }).catch((eror)=>console.log(">>retrieveServices",eror))
                BleManager.startNotification(peripheral.id, service, write).then((res) => {
                  console.log('Started notification on ' + res);
                  setTimeout(() => {
                    // var asciValue = [0x67, 0x65, 0x74, 0x53, 0x65, 0x72, 0x69, 0x61, 0x6c, 0x5c, 0x72];
                    var asciValue = [0xC0,103, 101, 116, 83, 101, 114, 105, 97, 108, 13,0xC0]
                    // console.log(">>1.",asciValue,"\n2."+this.getAsciValue('getSerial'))
                    BleManager.write(peripheral.id, service, read,asciValue).then((resWrite) => {
